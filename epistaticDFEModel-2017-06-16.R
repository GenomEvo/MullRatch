...
# Fitness Of Mutations and The Epistatic Model----

# The following function calculates fitness using an epistatic model and then 
# assigns this fitness value to all individuals within the population found to 
# have a single mutation.

# The variable pop is a vector containing the count of mutations each 
# individual carries and beta parameterises epistasis.
...
fitness.vector             <- function(pop, beta = -4) 
{
  ...
  # Location of individuals with a total of one mutation.
  ...
  # Find individuals with mutations and replace their fitness with the inherent
  # fitness cbind matrix.
  
  if (length(loc) != 0) 
  { 
    for (i in 1:length(SumPerIndividual))
    {
      # In the case of there being only one individual with a total of one 
      # mutation, use the following to replace their fitness with the inherent
      # fitness.
      ...
      # In the case of there being two or more individuals with a total of one 
      # mutation, use the following to replace their fitness with the inherent
      # fitness cbind matrix.
...
# Adding New Mutations To Each Mutation Type ---- 

# The following function adds new mutations to each mutation class (weak, mild
# and strong). Then combines the individual vectors, each associated with a
# mutation class, into a single vector.

# Include an explanation of N is population size; v1, v2, v3 are the 
# probability that a given new mutation is weak, mild or strong) and u is the 
# human per haploid genome mutation rate modified from Keightley (2012).
                                      ...
                                      u  = 0.175)
{
  ...
  # Combines the individual vectors generated above into a single matrix with
  # specified row names. This matrix can then be returned.
                                         ...
                                         kmax       = 1000) 
{
  ...
  # Naming rows and columns of popstates of each mutation class.
