...
plot_colours <- matrix(c(4,1,2), nrow= 1, ncol = 3)
...
for (n  in 1:length(gen)) {
  # Read each replicate
  rep1 <- readRDS(paste("NCS_", gen[n], "_rep1", sep = ""))
  rep2 <- readRDS(paste("NCS_", gen[n], "_rep2", sep = ""))
  rep3 <- readRDS(paste("NCS_", gen[n], "_rep3", sep = ""))
...
legend(x = "topright",inset = 0, legend = c("Weak", "Mild", "Strong"), col=plot_colours, lwd=5, cex=1,  bty="n", xpd=NA)
