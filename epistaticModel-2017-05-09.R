...
ratchet.simulator<-function(N=400, s=0.01, u=0.02, timepoints=seq(0,1000,by=10), kmax=1000, beta = -0.3 ) 
{
  ...
  fitness.vector<- exp((log(1-s*(0:kmax)*beta)/beta))     # vector of fitness values for each number of mutations
  ...
  xlist <- list(w, popstates)
  return(xlist)							 # return the list of popstates
}
...
xlist<-ratchet.simulator(timepoints=time.points)
pop.states<- xlist[[2]]
