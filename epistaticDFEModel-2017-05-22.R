...
{
  selection <- matrix(c(4.45E-05, 0.0032, 0.0218), nrow=3, ncol=1) #Selection values for the three groups
  
  #Beta parameterises epistasis
  oneMutation <- which(pop == 1, arr.in=TRUE)
  
  if (length(oneMutation) == 2) {                                  #Only one individual has at least one mutation in either weak, mild or strong
    SumPerIndividual <- which(sum(pop[oneMutation[,1],]) == 1)
  }
  else {                                                           #Two or more individuals have at least one mutation in either weak, mild or strong
