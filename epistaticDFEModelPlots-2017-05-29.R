...
for (n  in 1:length(gen_10000)) {
  ...
  matplot(x = names(fitness[[1]]) ,y = fitness[[1]],  type = "l", xlab = "Time (generations)", ylab = "log w")
  for (j in 2:length(pop.states)) {
    lines(x = names(fitness[[j]]), y =  fitness[[j]])
  }
  dev.off()
  ...
  matplot(x=rownames(pop.states[[1]]),y= pop.states[[1]], type="l",  col = c.specific,  xlab="Time (generations)", ylab="Number of ratchet clicks")
  for (j in 2:length(pop.states)) {
    lines(x = rownames(pop.states[[j]]), y =  pop.states[[j]][,1], col=plot_colours[1],lwd = 1.5)
    lines(x = rownames(pop.states[[j]]), y =  pop.states[[j]][,2], col=plot_colours[2],lwd = 1.5)
    lines(x = rownames(pop.states[[j]]), y = pop.states[[j]][,3], col=plot_colours[3],lwd = 1.5)
  }
  ...
  matplot(x=pop.states[[1]],y=fitness[[1]], type="l", col = c.specific, xlab="Number of ratchet clicks", ylab="log w")
  for (j in 2:length(pop.states)) {
    lines(x = pop.states[[j]][,1], y =  fitness[[j]], col=plot_colours[1],lwd = 1.5)
    lines(x = pop.states[[j]][,2], y =  fitness[[j]], col=plot_colours[2],lwd = 1.5)
    lines(x = pop.states[[j]][,3], y = fitness[[j]], col=plot_colours[3],lwd = 1.5)
  }
  ...
  table.name <- paste("mType_NCS_", gen_10000[n], sep = "")
